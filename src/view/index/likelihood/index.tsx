...
import { RecommendMessage } from '../../common/type';

type propsType = {
  recommendInfo: RecommendMessage | undefined;
};

function LikeLiHood(props: propsType) {
  const { recommendInfo } = props;
  useWhyDidYouUpdate('LikeLiHood', { ...props });
  return (
          ...
          <div>
            ...
            <div>
              <div className="mt-1 text-xs mb-3">
                <span>{recommendInfo?.cards[0].recommendName}</span>
              </div>
              <div className="flex items-center">
                <div className="text-red-500 font-semibold">
                  <span>¥</span>
                  <span>{recommendInfo?.cards[0].recommendPrice}</span>
                </div>
                <div className="text-gray-400 text-xs flex-1 text-right">
                  <span>已售{recommendInfo?.cards[0].recommendSale}件</span>
                </div>
              </div>
            </div>
          </div>
